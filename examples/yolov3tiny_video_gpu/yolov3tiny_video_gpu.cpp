...
#ifdef USE_OPENCV
void yolov3tinyOpencvVideo_GPU(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& labelsPath)
{
    try
    {
...
void yolov3tinyMsnhCVVideo_GPU(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& labelsPath)
{
    try
    {
        Msnhnet::NetBuilder  msnhNet;
        ...
        while (1)
        {
            ...
            auto st = Msnhnet::TimeUtil::startRecord();
            std::vector<float> img = Msnhnet::CVUtil::getPaddingZeroF32C3(mat, {inSize.x,inSize.y});
            std::vector<std::vector<Msnhnet::YoloBox>> result = msnhNet.runYoloGPU(img);
            std::cout<<"time  : " << Msnhnet::TimeUtil::getElapsedTime(st) <<"ms"<<std::endl;
...
#ifdef USE_OPENCV
    yolov3tinyOpencvVideo_GPU(msnhnetPath, msnhbinPath, labelsPath);
#elif USE_MSNHCV_GUI
    yolov3tinyMsnhCVVideo_GPU(msnhnetPath, msnhbinPath, labelsPath);
#endif
