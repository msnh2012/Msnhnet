...
#ifdef USE_OPENCV
void mobilenetYolov3LiteGPUOpencv(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath, const std::string& labelsPath)
{
	try
	{
...
void mobilenetYolov3LiteGPUMsnhCV(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath, const std::string& labelsPath)
{
	try
	{
		Msnhnet::NetBuilder  msnhNet;
...
int main(int argc, char** argv) 
{
    ...
    std::string msnhnetPath = std::string(argv[1]) + "/mobilenetv2_yolov3_lite/mobilenetv2_yolov3_lite.msnhnet";
    std::string msnhbinPath = std::string(argv[1]) + "/mobilenetv2_yolov3_lite/mobilenetv2_yolov3_lite.msnhbin";
    std::string labelsPath  = "../labels/coco.names";
    std::string imgPath = "../images/dog.jpg";
#ifdef USE_OPENCV
	mobilenetYolov3LiteGPUOpencv(msnhnetPath, msnhbinPath, imgPath, labelsPath);
#else
	mobilenetYolov3LiteGPUMsnhCV(msnhnetPath, msnhbinPath, imgPath, labelsPath);
#endif
    return 0;
}
