...
#ifdef USE_OPENCV
void classifyOpencv(const std::string &root ,const std::string &imgPath)
{
    try
    {
...
void classifyMsnhCV(const std::string &root ,const std::string &imgPath)
{
    try
    {
...
#ifdef USE_OPENCV
    classifyOpencv(root, imgPath);
#else
    classifyMsnhCV(root, imgPath);
#endif
