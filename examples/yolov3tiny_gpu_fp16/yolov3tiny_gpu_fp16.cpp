...
#ifdef USE_OPENCV
void yolov3tinyGPUOpencv(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath, const std::string& labelsPath)
{
	try
	{
...
void yolov3tinyGPUMsnhCV(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath, const std::string& labelsPath)
{
	try
	{
		Msnhnet::NetBuilder  msnhNet;
		...
		org.saveImage("yolov3tiny_gpu_fp16.jpg");
		#ifdef _WIN32
        system("yolov3tiny_gpu_fp16.jpg");
        #else
        std::cout<<"result pic has been saved at /[App Dir]/yolov3tiny_gpu_fp16.jpg"<<std::endl;
        #endif
	}
	catch (Msnhnet::Exception ex)
	{
...
#ifdef USE_OPENCV
	yolov3tinyGPUOpencv(msnhnetPath, msnhbinPath, imgPath, labelsPath);
#else
	yolov3tinyGPUMsnhCV(msnhnetPath, msnhbinPath, imgPath, labelsPath);
#endif
    return 0;
}
