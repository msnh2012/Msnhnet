...
#ifdef USE_OPENCV
void classifyGPUOpencv(const std::string &root ,const std::string &imgPath)
{
...
void classifyGPUMsnhCV(const std::string &root ,const std::string &imgPath)
{

    try
    {
		std::vector<float> img = Msnhnet::CVUtil::getImgDataF32C3(imgPath, { 227,227 });
        std::string config_file_ = root + "/alexnet/alexnet.msnhnet";
        Msnhnet::NetBuilder  msnhNet;
        // ================================ check alexnet ================================
        msnhNet.buildNetFromMsnhNet(config_file_);
        ...
        // =============================== check darknet53 ==============================
		img = Msnhnet::CVUtil::getImgDataF32C3(imgPath, { 224,224 });
        config_file_ = root + "/darknet53/darknet53.msnhnet";
        msnhNet.buildNetFromMsnhNet(config_file_);
        msnhNet.loadWeightsFromMsnhBin(root + "/darknet53/darknet53.msnhbin");
        ...
        // =============================== check googLenet ==============================
		img = Msnhnet::CVUtil::getGoogLenetF32C3(imgPath, { 224,224 });
        config_file_ = root + "/googLenet/googLenet.msnhnet";
        msnhNet.buildNetFromMsnhNet(config_file_);
        msnhNet.loadWeightsFromMsnhBin(root + "/googLenet/googLenet.msnhbin");
...
#ifdef USE_OPENCV
    classifyGPUOpencv(root, imgPath);
#else
    classifyGPUMsnhCV(root, imgPath);
#endif
    return 0;
}
