...
#ifdef USE_OPENCV
void yolov3tinyOpencv(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath,  const std::string& labelsPath)
{
    try
    {
...
void yolov3tinyMsnhCV(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath,  const std::string& labelsPath)
{
    try
    {
        Msnhnet::NetBuilder  msnhNet;
        ...
        std::cout<<msnhNet.getTimeDetail()<<std::endl<<std::flush;
        org.saveImage("yolov3tiny.jpg");
		#ifdef _WIN32
        system("yolov3tiny.jpg");
        #else
        std::cout<<"result pic has been saved at /[App Dir]/yolov3tiny.jpg"<<std::endl;
        #endif
    }
    catch (Msnhnet::Exception ex)
    {
...
#ifdef USE_OPENCV
    yolov3tinyOpencv(msnhnetPath, msnhbinPath, imgPath,labelsPath);
#else
    yolov3tinyMsnhCV(msnhnetPath, msnhbinPath, imgPath, labelsPath);
#endif
    return 0;
}
