...
#ifdef USE_OPENCV
void yolov5mGPUOpencv(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath,  const std::string& labelsPath)
{
    try
    {
...
void yolov5mGPUMsnhCV(const std::string& msnhnetPath, const std::string& msnhbinPath, const std::string& imgPath,  const std::string& labelsPath)
{
    try
    {
        Msnhnet::NetBuilder  msnhNet;
		...
		#ifdef _WIN32
        system("yolov5m_gpu.jpg");
        #else
        std::cout<<"result pic has been saved at /[App Dir]/yolov5m_gpu.jpg"<<std::endl;
        #endif
    }
    catch (Msnhnet::Exception ex)
    {
...
int main(int argc, char** argv)
{
    if(argc != 2)
    {
        std::cout<<"\nYou need to give models dir path.\neg: yolov5m /your/models/dir/path/ \n\nModels folder must be like this:\nmodels\n  |-Lenet5\n    |-Lenet5.msnhnet\n    |-Lenet5.msnhbin";
        getchar();
        return 0;
    }

    std::string msnhnetPath = std::string(argv[1]) + "/yolov5m/yolov5m.msnhnet";
    std::string msnhbinPath = std::string(argv[1]) + "/yolov5m/yolov5m.msnhbin";
    std::string labelsPath  = "../labels/coco.names";
    std::string imgPath = "../images/dog.jpg";
#ifdef USE_OPENCV
    yolov5mGPUOpencv(msnhnetPath, msnhbinPath, imgPath,labelsPath);
#else
    yolov5mGPUMsnhCV(msnhnetPath, msnhbinPath, imgPath, labelsPath);
#endif
    getchar();
