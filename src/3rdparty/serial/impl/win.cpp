...
#include "Msnhnet/3rdparty/serial/impl/win.h"

using std::string;
using std::wstring;
using std::stringstream;
using std::invalid_argument;
using Msnhnet::Serial;
using Msnhnet::Timeout;
using Msnhnet::bytesize_t;
using Msnhnet::parity_t;
using Msnhnet::stopbits_t;
using Msnhnet::flowcontrol_t;
using Msnhnet::SerialException;
using Msnhnet::PortNotOpenedException;
using Msnhnet::IOException;
...
void
Serial::SerialImpl::setTimeout (Msnhnet::Timeout &timeout)
{
  timeout_ = timeout;
  if (is_open_) {
    reconfigurePort ();
...
void
Serial::SerialImpl::setBytesize (Msnhnet::bytesize_t bytesize)
{
  bytesize_ = bytesize;
  if (is_open_) {
    reconfigurePort ();
...
void
Serial::SerialImpl::setParity (Msnhnet::parity_t parity)
{
  parity_ = parity;
  if (is_open_) {
    reconfigurePort ();
...
void
Serial::SerialImpl::setStopbits (Msnhnet::stopbits_t stopbits)
{
  stopbits_ = stopbits;
  if (is_open_) {
    reconfigurePort ();
...
void
Serial::SerialImpl::setFlowcontrol (Msnhnet::flowcontrol_t flowcontrol)
{
