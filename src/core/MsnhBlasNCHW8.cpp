...
void BlasNCHW8::cpuNCHWToNCHW8(float * const &org, const int width, const int height, const int channel, const int batch, float * const &dst)
{
    int outWidth   = 0;
        ...
        for (int oc = 0; oc < outChannel; ++oc)
        {
            for (int oh = 0; oh < outHeight; ++oh)
            {
                for (int ow = 0; ow < outWidth; ++ow)
                {
                    ...
                    {
                        dst[b*outChannel*outHeight*outWidth + oc*outHeight*outWidth + oh*outWidth + ow] = org[b*channel*height*width + oc*NCHW8_PACK*width*height + (ow%NCHW8_PACK)*width*height + ow/NCHW8_PACK +  oh*width];
                    }
                }
            }
        }
