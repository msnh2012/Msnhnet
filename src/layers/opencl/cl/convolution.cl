...
__kernel void IM2COL_GEMM_group (const int M, const int N, const int K, const __global float* A,  // K = sizeW * sizeH      
                      const __global float* B, __global float* C)
{
    const int globalRow = get_global_id(0);
    const int globalCol = get_global_id(1);
    ...
    float acc = 0.0f;
    for (int k = 0; k < K; k++) {
        acc += A[globalRow * K + k] * B[(globalRow * K + k) * N + globalCol];

        if (globalCol == 56 * 1 + 1 && globalRow == 1) {
            printf("k : %d\tA : %f\tB : %f\n", k, A[globalRow * K + k], B[(globalRow * K + k) * N + globalCol]);
            // printf("k : %d\tA : %f\tB : %f\n", k, A[k], B[k*N]);
        }
    }
    C[globalRow * N + globalCol] = acc;
...
__kernel void convWinograd_depthwise(const int inWidth,
                            const int inHeight,
                            const int inChannel,
                            const int outWidth,
                            const int outHeight,
                            ...
                            __global float* dst)
{
    ...
    __global float* dst0 = dst + chIdx * outWidth * outHeight;
    ...
    for(int i = 0; i < outHeight / 6; i++){
        for(int j = 0; j < outWidth / 6; j++){
            const __global float* r0 = src + chIdx * inWidth * inHeight + i * 6 * inWidth + j * 6;
            __global float* outputPtr = dst0 + i * 6 * outWidth + j * 6;
            ...
            for (int m = 0; m < 8; m++) {
                const __global float* filter0 = filter + chIdx * 8 * 8 + m * 8;
                float d0 = (tmpV[m][0] - tmpV[m][6] + (tmpV[m][4] - tmpV[m][2]) * 5.25f) * filter0[0];
                float d7 = (tmpV[m][7] - tmpV[m][1] + (tmpV[m][3] - tmpV[m][5]) * 5.25f) * filter0[7];
