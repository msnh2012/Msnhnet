    ...
    for (size_t i = 0; i < branchParams.size(); ++i)
    {
        ...
        for (size_t j = 0; j < branchParams[i].size(); ++j)
        {
            ...
            else if(branchParams[i][j]->type == LayerType::BATCHNORM)
            {
                ...
                layer                           =   new BatchNormLayer(branchBuildParams.batch, branchBuildParams.width, branchBuildParams.height, branchBuildParams.channels,
                                                                       batchNormParams->activation, batchNormParams->eps, batchNormParams->actParams);
                if(i == 0 && j == 0)
                {
                    this->_inputNum = layer->getInputNum();
                }
...
void AddBlockLayer::saveAllWeights(const int &mainIdx, const int &branchIdx, const int &branchIdx1)
{
    for (size_t i = 0; i < branchLayers.size(); ++i)
    {
        for (size_t j = 0; j < branchLayers[i].size(); ++j)
