...
int runClassifyFile(char **msg, const char *imagePath, int *bestIndex, PredDataType preDataType, int runGPU, const float* mean, const float *std)
{
    try
    {
        ...
        case PRE_DATA_NONE:
            ...
            break;
        case PRE_DATA_FC32_C1:
            img = Msnhnet::CVUtil::getImgDataF32C1(imagePath, {sizeX, sizeY});
            break;
        case PRE_DATA_FC32_C3:
            img = Msnhnet::CVUtil::getImgDataF32C3(imagePath, {sizeX, sizeY});
            break;
        case PRE_DATA_GOOGLENET_FC3:
            img = Msnhnet::CVUtil::getGoogLenetF32C3(imagePath, {sizeX, sizeY});
            break;
        case PRE_DATA_PADDING_ZERO_FC3:
            img = Msnhnet::CVUtil::getPaddingZeroF32C3(imagePath, {sizeX, sizeY});
            break;
        case PRE_DATA_CAFFE_FC3:
            img = Msnhnet::CVUtil::getCaffeModeF32C3(imagePath, {sizeX, sizeY});
            break;
        case PRE_DATA_TRANSFORMED_FC3:
            if(mean == nullptr || std == nullptr)
            {
            ...
            img = Msnhnet::CVUtil::getTransformedF32C3(imagePath, {sizeX, sizeY},{mean[0], mean[1], mean[2]},{std[0], std[1], std[2]});
            break;
        }
                    ...
                    int *bestIndex, PredDataType preDataType, const int swapRGB,  int runGPU, const float *mean, const float *std)
{
    try
    {
        ...
        Msnhnet::Mat inMat(width, height, ((channel)==1)?Msnhnet::MAT_GRAY_U8:Msnhnet::MAT_RGB_U8,imageData);

        if(!swapRGB)
        {
            Msnhnet::MatOp::cvtColor(inMat,inMat,Msnhnet::CvtColorType::CVT_RGB2BGR);
        }
        ...
        case PRE_DATA_NONE:
            ...
            break;
        case PRE_DATA_FC32_C1:
            img = Msnhnet::CVUtil::getImgDataF32C1(inMat, {sizeX, sizeY});
            break;
        case PRE_DATA_FC32_C3:
            img = Msnhnet::CVUtil::getImgDataF32C3(inMat, {sizeX, sizeY});
            break;
        case PRE_DATA_GOOGLENET_FC3:
            img = Msnhnet::CVUtil::getGoogLenetF32C3(inMat, {sizeX, sizeY});
            break;
        case PRE_DATA_PADDING_ZERO_FC3:
            img = Msnhnet::CVUtil::getPaddingZeroF32C3(inMat, {sizeX, sizeY});
            break;
        case PRE_DATA_CAFFE_FC3:
            img = Msnhnet::CVUtil::getCaffeModeF32C3(inMat, {sizeX, sizeY});
            break;
        case PRE_DATA_TRANSFORMED_FC3:
            if(mean == nullptr || std == nullptr)
            {
            ...
            img = Msnhnet::CVUtil::getTransformedF32C3(inMat, {sizeX, sizeY},{mean[0], mean[1], mean[2]},{std[0], std[1], std[2]});
            break;
        }
...
int runYoloFile(char **msg, const char *imagePath, BBoxContainer *bboxContainer, int *detectedNum, const int runGPU)
{
    try
    {
        ...
        std::vector<float> img = Msnhnet::CVUtil::getPaddingZeroF32C3(imagePath,{sizeX,sizeY});
...
int runYoloList(char **msg, unsigned char *imageData, const int width, const int height, const int channel, BBoxContainer *bboxContainer,  int *detectedNum, const int swapRGB, const int runGPU)
{
    try
    {
        ...
        std::vector<float> img = Msnhnet::CVUtil::getPaddingZeroF32C3(inMat,{sizeX,sizeY});
